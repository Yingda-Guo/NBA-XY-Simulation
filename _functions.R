...
get_pbp_file <- function(file_name){
  #Grabs the play by play data from the NBA site
  the.data.file<-fromJSON(file_name)
  test <-the.data.file$resultSets$rowSet
  test2 <- test[[1]]
  test3 <- data.frame(test2)
  coltest <- the.data.file$resultSets$headers
...
chull_plot <- function(df, eventid, gameclock) {
  ...
  df_hull2 <- df2 %>% filter(ID == min(ID, na.rm = T)) %>% select(X,Y)
  df_hull3 <- df2 %>% filter(ID == max(ID, na.rm = T)) %>% select(X,Y)
...
velocity_one_direction <- function(loc){
  diff <- as.vector(diff(loc))
  sum(diff)
}
...
angle_vectors <- function(a, b){
  # Function to calculate angle of two vector
